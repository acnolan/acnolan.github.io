...
const snowStorm = (function (window, document) {
  ...
  this.events = (function () {
    const old = (!window.addEventListener && window.attachEvent); const { slice } = Array.prototype;
    const evt = {
    ...
    function getArgs(oArgs) {
      const args = slice.call(oArgs); const
        len = args.length;
      if (old) {
  ...
  this.SnowFlake = function (type, x, y) {
    ...
    const snowType = Math.floor((Math.random() * 2));
    if (snowType === 0) {
      this.o.innerHTML = 'M';
    } else {
      this.o.innerHTML = 'A';
  ...
  this.snow = function () {
    let active = 0; let flake = null; let i; let
      j;
